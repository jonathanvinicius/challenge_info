...
export class FilterExceptionInterceptor implements ExceptionFilter {
  private readonly logger = new Logger(FilterExceptionInterceptor.name);
    ...
    if (statusLogger.includes(status) || status) {
      ...
      const customMessage = LoggerService.buildMessage('E', status, msg);
      this.logger.error(customMessage, JSON.stringify(error));
    }
    ...
    if (exception.class && exception.code === 'EREQUEST') {
      const precedingErrors =
        exception.precedingErrors?.length > 0
          ? exception.precedingErrors
          : undefined;
      if (precedingErrors) {
        this.logger.error('precedingErrors', precedingErrors);
      }
    }
    ...
    this.logger.error(`Exception ${JSON.stringify(data)}`);
